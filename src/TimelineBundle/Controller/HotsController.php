    ...
    public function getDataFromHots($playerName,$idHots, $accountId)
    {
        ...
        if (!empty($arrayId[0])) {
            ...
            for ($i = 1; $i <= 2; $i++) {
                ...
                $arrayGame = array('Game' => $idHots, 'idComm' => $dateMatch, 'player' => $playerName, 'accountId' => $accountId, 'heroName' => $heroName, 'mapName' => $mapName, 'timeGame' => $timeGame, 'levelPlayer' => $levelPlayer, 'currentMmr' => $currentMmr, 'varMmr' => $varMmr, 'createDate' => $dateMatch, 'resultGame' => $resultGame);
